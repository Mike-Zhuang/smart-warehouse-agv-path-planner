...
namespace
{
        ...
        for (int time_step = 0; time_step < makespan; ++time_step)
        {
            for (std::size_t first = 0; first < paths.size(); ++first)
            {
                for (std::size_t second = first + 1; second < paths.size(); ++second)
                {
                    ...
                    {
                        return Conflict{
                            first, second, ConstraintType::Edge, time_step, {}, first_previous, first_point, second_previous, second_point};
                    }
                }
            }
        }
    ...
    Constraint make_constraint(const Conflict &conflict, const std::size_t robot_index)
    {
        ...
        {
            return Constraint{
                robot_index, ConstraintType::Edge, conflict.time_step, {}, conflict.first_from, conflict.first_to};
        }
        return Constraint{
            robot_index, ConstraintType::Edge, conflict.time_step, {}, conflict.second_from, conflict.second_to};
    }
