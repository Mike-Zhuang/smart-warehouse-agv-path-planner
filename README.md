...
地图顶部还支持 `2D 平面 / 3D 仓库` 切换。3D 视图使用 Three.js 在浏览器端渲染，
复用同一份地图、任务和动画状态，点击 3D 地砖也可以编辑地图或设置多车任务点。模型
资源位于 `frontend/public/models/`，当前默认使用程序化货架、托盘障碍和叉车式 AGV，
并保留 GLB 替换位置。3D 视图会展示搜索扩展、贴地路线和车辆动画。
...
- 默认提供 `20 × 15` 仓库网格，并支持自定义行列数。
- 地图元素覆盖 `0` 通道、`1` 货架、`2` 临时障碍、`3` 装卸区、`4` 目标货架。
- C++ 核心实现标准 A*，启发函数使用曼哈顿距离。
- 支持上下左右和四个对角方向移动，默认禁止斜向穿越墙角。
- 支持从装卸区到目标货架，再返回装卸区的往返路径规划。
- 输出最优路径、路径长度和搜索节点数量。
- 前端提供 2D / 3D 可视化，能够展示搜索过程和最终路径。
